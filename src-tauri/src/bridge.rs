...
pub fn validate_origins(instance_origins: &[String]) -> Vec<String> {
    ...
    for origin in instance_origins {
        if let Ok(origin_url) = Url::parse(origin) {
            if patterns
                .iter()
                .any(|p| pattern_matches_origin(p, &origin_url))
            {
...
pub fn emit_key_inject(keysym: u32, down: bool) -> Result<(), BridgeError> {
    emit_to_main(EVENT_KEY_INJECT, KeyInjectPayload { keysym, down })
}
...
pub fn emit_session_command(
    ...
    emit_to_main(EVENT_SESSION_COMMAND, SessionCommandPayload { cmd, arg })
}
...
fn emit_to_main(event: &str, payload: impl Serialize + Clone) -> Result<(), BridgeError> {
    ...
    handle
        .emit_to(
            EventTarget::webview_window(SHELL_WINDOW_LABEL),
            event,
            payload,
        )
...
mod tests {
    ...
    fn tauri_conf_enables_the_global_tauri_api() {
        let raw: serde_json::Value = serde_json::from_str(include_str!("../tauri.conf.json"))
    ...
    fn pattern_matching_is_exact_by_default() {
        ...
        assert!(pattern_matches_origin(
            "https://persea.example.com:8443",
            &origin
        ));
        assert!(pattern_matches_origin(
        ...
        assert!(!pattern_matches_origin(
            "http://persea.example.com:8443",
            &origin
        ));
        assert!(!pattern_matches_origin(
            "https://persea.example.com:9443",
            &origin
        ));
    }
